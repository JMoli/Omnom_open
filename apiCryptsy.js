...
module.exports = function() {
	...
	function Cryptsy(key, secret){
		...
		this._getPrivateHeaders = function(parameters){
			...
			signature = new crypto('sha512', secret).update(formatParam(parameters)).digest('hex');
			return {
				Key: key,
					Sign: signature
...
//INTERNAL FUNCTIONS AKA NOT API CALLS
		getTop: function(currencyA, currencyB, callback){
			this.getSingleMarket(currencyA, currencyB, function(err, data){
				data = JSON.parse(data);
				data = data.return.markets[currencyA].buyorders;
				callback(err,data);
			});
		},
		getBottom: function(currencyA, currencyB, callback){
			this.getSingleMarket(currencyA, currencyB, function(err, data){
				data = JSON.parse(data);
				data = data.return.markets[currencyA].sellorders;
				callback(err, data);
			});
		},
		getFunds: function(currency, callback){
			this.myBalances(function(err, data){
				data = JSON.parse(data);
				data = data.return.balances_available[currency];	
				callback(err, data);
			});
		},
		getWallet: function(callback){
			this.myBalances(function(err, data){
				data = JSON.parse(data);
				data = data.return.balances_available;
				callback(err, data);
			});
		},
		//returns an obj array: if item sold -sold{ id:amount}, if not sold - open{id:amountremaining} 
		confirmSale: function(currencyA, currencyB, sellOrders, timeStamp, callback){
			var main = this;
			var sold = {};
			var open = {};
			var reply = [];
			...
			async.each(sellOrders, function(id, callback){
				main.getOrderStatus(id, function(err, data){
					data = JSON.parse(data);
					if(data.return.tradeinfo[0] != undefined){
					totalBtc = totalBtc + (parseFloat(data.return.tradeinfo[0].total) - parseFloat(data.return.tradeinfo[0].fee));
					}
					if(data.return.orderinfo.active == false){ //it sold
						sold[id] = parseFloat(data.return.tradeinfo[0].total) - parseFloat(data.return.tradeinfo[0].fee);
					}else if(data.return.orderinfo.active == true){
						open[id] =  data.return.orderinfo.remainqty;
					}
					callback(null);
				});
			}, function(err){
				response(sold, open, totalBtc);
			});
		},
		sellDump: function(coin, market, quantity, rate, callback){

		},
		createSale: function(coin, market, quantity, rate, type, callback){
			this.createOrder(coin, market, 'sell', quantity, rate, function(err, data){
				console.log(data);
				data = JSON.parse(data);
				if(data.success == 0){ callback(data.error, null); console.log(data);}
				else{data = data.orderid;
				callback(null, data);}
			});
		},
		cancel: function(coin, market, uuid, callback){
			this.cancelOrder(uuid, function(err, data){
				console.log(data);
				//TODO figure out what return is if failed
				callback(err, true);
			});
		}
	};
